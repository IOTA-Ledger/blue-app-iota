...
void get_bundle_essence_trits(const char address[], uint64_t value,
                              const char tag[], uint64_t timestamp,
                              uint32_t current_index, uint32_t last_index,
                              trit_t bundle_essence_trits[]);
                              ...
                              trit_t bundle_essence_trits[])
{
    ...
    // Last index
    {
        trit_t *last_idx_trits =
            &bundle_essence_trits[3 * (81 + 27 + 27 + 9 + 9)];
        int32_to_trits(last_index, last_idx_trits, 3 * 9);
    }
}

void calculate_standard_bundle_hash(
    const char from_addr[], const char to_addr[], const char remainder_addr[],
    uint64_t balance, uint64_t transaction_amount, const char tag[],
    uint64_t timestamp, tryte_t bundle_hash_out[])
{
    ...
    while (!valid_bundle) {
        ...
        get_bundle_essence_trits(to_addr, transaction_amount, obsolete_tag_char,
                                 timestamp, 0, 3, bundle_essence_trits);
        kerl_absorb_trits(&sha3, bundle_essence_trits, 486);
        get_bundle_essence_trits(from_addr, -balance, tag, timestamp, 1, 3,
                                 bundle_essence_trits);
        kerl_absorb_trits(&sha3, bundle_essence_trits, 486);
        get_bundle_essence_trits(from_addr, 0, tag, timestamp, 2, 3,
                                 bundle_essence_trits);
        kerl_absorb_trits(&sha3, bundle_essence_trits, 486);
        get_bundle_essence_trits(remainder_addr, balance - transaction_amount,
                                 tag, timestamp, 3, 3, bundle_essence_trits);
        kerl_absorb_trits(&sha3, bundle_essence_trits, 486);
...
void generate_signature_fragment(tryte_t normalized_bundle_hash_fragment[],
                                 trit_t private_key_fragment[],
                                 trit_t signature_fragment[])
{
